...
export function BeautyApp({ page, data }: { page: Page; data: PageData }) {
 ...
 return <div className={`app-shell ${page === 'home' ? 'home-page' : page === 'history' ? 'history-page' : 'info-page'}`}>
  ...
  <Modal open={qrOpen} setOpen={setQrOpen} title="Kartu member kamu" description="Tunjukkan barcode ini kepada kasir Beauty Kendari."><div className="large-qr"><Barcode128 value={member.card} width={280} height={96} title={`Nomor kartu ${member.card}`} /></div><div className="qr-details"><strong>{member.name}</strong><span>{member.card}</span>{demo && <span className="demo-disclaimer">Kartu contoh. Bukan kartu member asli.</span>}</div><button className="button primary full" onClick={copyCard}>{copied ? <Check size={19} /> : <Copy size={19} />}{copied ? 'Nomor tersalin' : 'Salin nomor kartu'}</button></Modal>
